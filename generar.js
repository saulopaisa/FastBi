// generar.js - Versión Final con Cartones Dispersos (Máx 3 coincidencias por posición)
...
// ============ GENERAR COLUMNA (MÁX 3 COINCIDENCIAS POR POSICIÓN) ============
...
function mostrarLista() {
    db.ref('salas/' + SALA_ID + '/cartones').once('value', function(snap) {
        ...
        const contenedor = document.getElementById('listaCartones'); contenedor.innerHTML = '';
        ...
        const cartones = []; snap.forEach(function(child) { cartones.push({ key: child.key, data: child.val() }); });
        ...
        cartones.forEach(function(item) {
            ...
            const div = document.createElement('div'); div.className = 'card-carton'; div.setAttribute('data-id', id);
...
function actualizarBtnAsignar() { const btn = document.getElementById('btnAsignar'); if (btn) { btn.textContent = seleccionados.size > 0 ? '👤 ASIGNAR (' + seleccionados.size + ' cartones)' : '👤 ASIGNAR A JUGADOR'; btn.style.display = seleccionados.size > 0 ? 'block' : 'none'; } }
    ...
    cargarRegistroDesdeFirebase(function() {
        db.ref('salas/' + SALA_ID + '/cartones').once('value', function(snap) {
            ...
            for (let i = 0; i < cantidad; i++) {
                ...
                }, function(error) { if (!error) { creados++; if (creados === cantidad) { mostrarLista(); input.value = ''; mostrarToast('✅ ' + cantidad + ' cartones generados'); } } });
            }
        });
    });
}
...
function asignarAJugador() {
    ...
    const nombre = prompt('👤 Nombre:'); if (!nombre || !nombre.trim()) return;
    const nj = nombre.trim(), ids = Array.from(seleccionados); let comp = 0;
    ids.forEach(function(id) { db.ref('salas/' + SALA_ID + '/cartones/' + id).update({ estado: 'asignado', asignadoA: nj }, function(error) { if (!error) { comp++; if (comp === ids.length) { seleccionados.clear(); mostrarLista(); mostrarAsignacionExitosa(nj, ids, generarLinkCifrado(nj, ids)); } } }); });
}
...
function mostrarAsignacionExitosa(nombre, ids, link) {
    ...
    document.getElementById('btnCopiarLink').addEventListener('click', function() { const inp = document.getElementById('linkJugadorInput'); inp.select(); navigator.clipboard.writeText(inp.value); this.textContent = '✅ COPIADO!'; this.style.background = '#10b981'; setTimeout(() => { this.textContent = '📋 COPIAR LINK'; this.style.background = '#3b82f6'; }, 2000); });
}

// ============ VISTA PREVIA ============
function verPreview(id) {
    const preview = document.getElementById('vista-previa-contenido');
    db.ref('salas/' + SALA_ID + '/cartones/' + id).once('value', function(snap) { const d = snap.val(); if (!d || !d.carton) return; let h = '<div style="padding:15px;"><h3>Cartón #' + d.numero + '</h3>'; if (d.asignadoA) h += '<p style="color:#10b981;">👤 ' + d.asignadoA + '</p>'; h += '<table style="width:100%;border-collapse:collapse;max-width:300px;margin:10px auto;"><tr style="background:#ff4d4d;color:white;"><th>B</th><th>I</th><th>N</th><th>G</th><th>O</th></tr>'; for (let f=0;f<5;f++) { h+='<tr>'; ['B','I','N','G','O'].forEach(function(l){ const v=d.carton[l][f],c=(l==='N'&&f===2); h+='<td style="padding:8px;border:2px solid #e2e8f0;text-align:center;'+(c?'background:#fef3c7;':'')+'">'+(c?'⭐':v)+'</td>'; }); h+='</tr>'; } h += '</table></div>'; preview.innerHTML = h; });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    inicializarRegistro(); cargarRegistroDesdeFirebase();
