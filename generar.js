...
function htmlCartonPDF(c) {
    ...
    for (let f=0;f<5;f++) { h+='<tr>'; ['B','I','N','G','O'].forEach(function(l){ const v=carton[l][f]; const centro=(l==='N'&&f===2); h+='<td class="'+(centro?'free':'')+'">'+(centro?'⭐':v)+'</td>'; }); h+='</tr>'; }
    h += '</table></div>';
    return h;
}
...
function abrirMenuPDF() {
    ...
    db.ref('salas/' + SALA_ID + '/cartones').once('value', function(snap) {
        ...
        html += '<button onclick="exportarPDFTodos()" style="width:100%;padding:15px;background:#ff4d4d;color:white;border:none;border-radius:10px;cursor:pointer;font-size:1.1rem;font-weight:bold;margin-bottom:25px;">📄 EXPORTAR TODOS (' + total + ')</button>';
        
        if (jugadores.size > 0) {
            html += '<h3 style="color:#1e293b;margin-bottom:15px;">👤 Por Jugador</h3><div style="max-height:50vh;overflow-y:auto;">';
            jugadores.forEach(function(jugador) {
                let count = 0;
                snap.forEach(function(child) { if (child.val().asignadoA === jugador) count++; });
                ...
                html += '<div><strong style="font-size:1rem;">👤 ' + jugador + '</strong><br><span style="color:#64748b;font-size:0.85rem;">' + count + ' cartón(es)</span></div>';
