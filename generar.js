...
function inicializarRegistro() {
    ['B','I','N','G','O'].forEach(function(letra) {
        for (let f = 0; f < 5; f++) {
            if (!registroPosiciones[letra + '-' + f]) registroPosiciones[letra + '-' + f] = {};
        }
    });
}
            ...
            if (c.carton) {
                ['B','I','N','G','O'].forEach(function(letra) {
                    c.carton[letra].forEach(function(valor, fila) {
                        if (!(letra === 'N' && fila === 2)) registrarPosicion(letra, fila, valor);
                    });
                });
            }
        });
...
function generarColumna(min, max, letra, fila) {
    ...
    for (let n = min; n <= max; n++) {
        ...
        if (count < 3) disponibles.push(n);
    }
    if (disponibles.length < 5) { disponibles.length = 0; for (let n = min; n <= max; n++) disponibles.push(n); }
    for (let i = disponibles.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [disponibles[i], disponibles[j]] = [disponibles[j], disponibles[i]]; }
    const seleccionados = disponibles.slice(0, 5).sort((a, b) => a - b);
    seleccionados.forEach(function(valor, idx) { if (!(letra === 'N' && idx === 2)) registrarPosicion(letra, idx, valor); });
...
function mostrarLista() {
    db.ref('salas/' + SALA_ID + '/cartones').once('value', function(snap) {
        ...
        cartones.forEach(function(item) {
            ...
            div.innerHTML = 
                '<div class="carton-item-header">' +
                '<span class="carton-item-numero"># ' + (c.numero||'?') + '</span>' +
                '<span class="carton-item-estado estado-' + (c.estado||'disponible') + '">' + (c.estado||'disponible') + '</span>' +
                '</div>' +
                (c.asignadoA ? '<div class="carton-item-jugador">👤 ' + c.asignadoA + '</div>' : '') +
                '<input type="text" class="carton-item-nombre" value="' + (c.nombre||'') + '" ' +
                'onchange="renombrarCarton(\'' + id + '\', this.value)" onclick="event.stopPropagation()" placeholder="Nombre...">' +
                '<div class="carton-item-acciones">' +
                '<button class="btn-mini select' + (sel ? ' activo' : '') + '" ' +
                'onclick="event.stopPropagation(); toggleSeleccion(\'' + id + '\')">' + (sel ? '✓' : '○') + '</button>' +
                '<button class="btn-mini link" onclick="event.stopPropagation(); copiarLink(\'' + id + '\')">🔗</button>' +
                '<button class="btn-mini estado" onclick="event.stopPropagation(); cambiarEstado(\'' + id + '\')">🔄</button>' +
                '<button class="btn-mini eliminar" onclick="event.stopPropagation(); eliminarCarton(\'' + id + '\')">🗑️</button>' +
                '</div>';
    ...
    cargarRegistroDesdeFirebase(function() {
        db.ref('salas/' + SALA_ID + '/cartones').once('value', function(snap) {
            ...
            for (let i = 0; i < cantidad; i++) {
                ...
                }, function(error) { if (!error) { creados++; if (creados === cantidad) { mostrarLista(); input.value = ''; mostrarToast('✅ ' + cantidad + ' cartones generados', 'success'); } } });
            }
        });
    });
}
...
function mostrarAsignacionExitosa(nombre, ids, link) {
    ...
    if (btnCopiar) {
        btnCopiar.addEventListener('click', function() {
            const inp = document.getElementById('linkJugadorInput');
...
function verPreview(id) {
    ...
    db.ref('salas/' + SALA_ID + '/cartones/' + id).once('value', function(snap) {
        ...
        h += '<table class="preview-tabla"><tr><th>B</th><th>I</th><th>N</th><th>G</th><th>O</th></tr>';
        for (let f=0;f<5;f++) { h+='<tr>'; ['B','I','N','G','O'].forEach(function(l){ const v=d.carton[l][f],c=(l==='N'&&f===2); h+='<td class="'+(c?'free':'')+'">'+(c?'⭐':v)+'</td>'; }); h+='</tr>'; }
...
function copiarLink(id) { navigator.clipboard.writeText(generarLinkCifrado('Cartón individual', [id])).then(() => mostrarToast('✅ Link copiado', 'success')); }
...
function borrarTodo() { if (confirm('⚠️ ¿Eliminar TODOS?')) { seleccionados.clear(); db.ref('salas/' + SALA_ID + '/cartones').remove(() => { mostrarLista(); mostrarToast('✅ Eliminados', 'success'); }); } }
...
function exportarPDFTodos() {
    db.ref('salas/'+SALA_ID+'/cartones').once('value',function(snap){
        ...
        const c=[];snap.forEach(function(ch){c.push(ch.val())});c.sort((a,b)=>(a.numero||0)-(b.numero||0));
        abrirVentanaImpresion(c,'Todos los Cartones');
    });
}

function exportarPDFPorJugador(nj) {
    db.ref('salas/'+SALA_ID+'/cartones').once('value',function(snap){
        const c=[];snap.forEach(function(ch){if(ch.val().asignadoA===nj)c.push(ch.val())});
        if(c.length===0){alert('No hay cartones para: '+nj);return}
        c.sort((a,b)=>(a.numero||0)-(b.numero||0));
        abrirVentanaImpresion(c,'👤 '+nj);
    });
}

function abrirVentanaImpresion(cartones,titulo) {
    const v=window.open('','_blank','width=800,height=600');
    v.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Bingo - '+titulo+'</title><style>'+getPDFStyles()+'</style></head><body>');
    for(let i=0;i<cartones.length;i+=2){
        v.document.write('<div class="pagina"><h1>🎯 BINGO PRO</h1><h2>'+titulo+'</h2><p class="info">Página '+(Math.floor(i/2)+1)+' | '+cartones.length+' cartones | '+new Date().toLocaleDateString()+'</p>');
        v.document.write(htmlCartonPDF(cartones[i]));
        if(i+1<cartones.length)v.document.write(htmlCartonPDF(cartones[i+1]));
        v.document.write('</div>');
    }
    v.document.write('<script>window.onload=function(){setTimeout(function(){window.print()},800)};<\/script></body></html>');
    v.document.close();
    mostrarToast('✅ Selecciona "Guardar como PDF"','success');
}
...
function abrirMenuPDF() {
    ...
    if (!p) return;
    db.ref('salas/'+SALA_ID+'/cartones').once('value',function(snap){
        const t=snap.numChildren()||0,j=new Set();
        snap.forEach(function(c){if(c.val().asignadoA)j.add(c.val().asignadoA)});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var cantidadGenerar = document.getElementById('cantidadGenerar');
    
    if (btnGenerar) btnGenerar.addEventListener('click', generarLote);
    if (btnIrJuego) btnIrJuego.addEventListener('click', function() { location.href = 'ruleta.html'; });
    if (btnAsignar) btnAsignar.addEventListener('click', asignarAJugador);
    if (fileIn) fileIn.addEventListener('change', importarJSON);
    if (buscador) buscador.addEventListener('input', function(e) { filtrarCartones(e.target.value); });
    if (cantidadGenerar) cantidadGenerar.addEventListener('keypress', function(e) { if (e.key === 'Enter') generarLote(); });
