...
// Referencias Firebase (Asegúrate de que 'db' esté inicializado en tu HTML)
...
// --- 3. FUNCIONES DE SINCRONIZACIÓN FIREBASE ---

function mandarPatronAlJuego(patronArray) {
    // Crea un mapa de 25 celdas (true/false) para enviar a los jugadores
    const dataParaFirebase = Array(25).fill(false);
    patronArray.forEach(idx => {
        if(idx !== null) dataParaFirebase[idx] = true;
    });

    db.ref('configuracion/patron').set(dataParaFirebase)
        .then(() => console.log("Patrón enviado a los jugadores"))
        .catch(e => console.error("Error Firebase Patrón:", e));
}
...
function mandarNumeroAlJuego(num) {
    db.ref('partidaActual').set({
        ...
        letra: getLetra(num),
        timestamp: Date.now()
    }).catch(e => console.error("Error Firebase Número:", e));
}

function reiniciarPartidaFirebase() {
    db.ref('partidaActual').set({ status: "reiniciado" });
    db.ref('configuracion/patron').set(Array(25).fill(false));
}

// --- 4. LÓGICA DE INTERFAZ Y PATRONES ---
...
// --- 7. MOTOR MATEMÁTICO (GENERACIÓN DETERMINISTA) ---
...
document.getElementById('resetBtn').onclick = () => { 
    if(confirm("¿Seguro? Se reiniciará la partida para todos.")) {
        reiniciarPartidaFirebase();
        location.reload();
    }
};
...
function renderMiniatura(id, apodo, matrix, win, container) {
    ...
    matrix.forEach((fila, r) => {
        html += "<tr>";
        fila.forEach((celda, c) => {
            const hit = (celda === "FREE" || drawnNumbers.includes(celda));
            html += `<td style="border:1px solid #ddd; padding:4px; background:${hit?'#ffb703':'none'};">${celda==='FREE'?'★':celda}</td>`;
        });
        html += "</tr>";
    });
    html += `</table></div>`;
