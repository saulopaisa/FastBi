...
function actualizarEtapas() {
    ...
    if (window.etapaActual >= 2) {
        ...
        if (cardJ) cardJ.classList.add('completado');
    ...
    if (window.etapaActual >= 3) {
        ...
        if (cardP) cardP.classList.add('completado');
...
window.programarJuego = function() {
    ...
    window.intervaloTemporizador = setInterval(function() {
        ...
        if (tiempo <= 0) {
            ...
            db.ref('partidas/' + SALA_ID).update({ estado: 'jugando', cronometro: 0, partidaIniciada: true, mensajeAdmin: '▶️ ¡Juego iniciado!', timestamp: Date.now() });
...
function actualizarMinicartonEnAmbasVistas(c) { 
    ...
    var contD = document.getElementById('minicartonVerificador'); if (contD) contD.innerHTML = html; 
    var contM = document.getElementById('minicartonMobile'); if (contM) contM.innerHTML = html; 
}
...
function verificarBingoAutomatico(c) { 
    ...
    if (el) { 
        if (bingo) { el.className = 'minicarton-estado estado-bingo'; el.textContent = '🎉 BINGO'; } 
        else { el.className = 'minicarton-estado'; el.textContent = 'Faltan: ' + faltantes.slice(0,3).join(', '); el.style.background = '#fef3c7'; el.style.color = '#92400e'; } 
    } 
    return bingo; 
}
...
function notificarBingoManual(c) { 
    ...
    mostrarToast('🚨 ¡BINGO cantado!', 'error'); 
...
window.bingoValido = function() { 
    ...
    if (confirm('¿BINGO VÁLIDO para ' + nombre + '?')) { 
        ...
        if (window.ganadoresPartida >= 2) { 
            ...
            db.ref('partidas/' + SALA_ID).update({ mensajeAdmin: '🎉 ¡BINGO! ' + nombre + ' - Aún queda 1 premio', timestamp: Date.now() }); 
    ...
    db.ref('partidas/' + SALA_ID + '/erroresJugadores/' + nombre).once('value', function(snap) {
        ...
        if (errores >= 2) {
            if (confirm('⚠️ ' + nombre + ' tiene ' + errores + ' errores.\n\n¿ELIMINAR jugador?')) {
                ...
                db.ref('partidas/' + SALA_ID).update({ estado: 'jugando', mensajeAdmin: '❌ ' + nombre + ' eliminado', timestamp: Date.now() });
                db.ref('partidas/' + SALA_ID + '/chat').push({ mensaje: '❌ ' + nombre + ' eliminado por 2 bingos errados', timestamp: Date.now(), admin: true, sistema: true });
                db.ref('partidas/' + SALA_ID + '/erroresJugadores/' + nombre).remove();
            ...
            if (confirm('⚠️ BINGO ERRADO para ' + nombre + ' (Error ' + errores + '/2)\n\n¿Confirmar?')) {
                finalizarRevision(nombre, 'errado');
                db.ref('partidas/' + SALA_ID).update({ estado: 'jugando', mensajeAdmin: '⚠️ BINGO ERRADO - ' + nombre + ' (' + errores + '/2)', timestamp: Date.now() });
                window.bingoDetectado = false; window.cerrarAlerta(); window.cartonActual = null;
